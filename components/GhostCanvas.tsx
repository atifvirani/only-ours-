...
export const GhostCanvas: React.FC<GhostCanvasProps> = ({ currentUser }) => {
  ...
  // Initialize Canvas Size to full screen
  useEffect(() => {
    const handleResize = () => {
  ...
  // Supabase Realtime Setup with Presence
  useEffect(() => {
    ...
    channel
      ...
      .on('presence', { event: 'sync' }, () => {
        ...
        const partners = Object.values(state)
          .flat() as unknown as PresenceState[];
        
        const partner = partners.find(p => p.user !== currentUser);
        if (partner) {
          setPartnerStatus({ 
            user: partner.user, 
            isDrawing: partner.isDrawing, 
            isOnline: true 
          });
        } else {
  ...
  // Update presence status when isDrawing changes
  useEffect(() => {
  ...
  const draw = (e: React.MouseEvent | React.TouchEvent) => {
    if (!isDrawing || !lastPoint || !penEnabled) return;
    const coords = getCoordinates(e);
    if (!coords) return;
    ...
    <div className="fixed inset-0 bg-transparent pointer-events-none overflow-hidden select-none">
      ...
      <canvas
        ...
        className={`absolute inset-0 block transition-opacity duration-300 ${
          penEnabled ? 'pointer-events-auto cursor-crosshair opacity-100' : 'pointer-events-none opacity-90'
        }`}
      />

      {/* Floating Status Notification (Partner) */}
      {partnerStatus && (
        <div className="fixed top-8 left-8 flex items-center space-x-3 bg-white/10 backdrop-blur-xl px-4 py-2 rounded-2xl border border-white/20 shadow-2xl animate-in slide-in-from-left duration-500">
          ...
          <div className="flex flex-col">
            <span className="text-white text-xs font-bold uppercase tracking-wider">{partnerStatus.user} is here</span>
            {partnerStatus.isDrawing && (
              <span className="text-rose-300 text-[10px] font-medium animate-pulse">Is currently drawing...</span>
            )}
          </div>
        </div>
      )}

      {/* Vertical Glassmorphic Toolbar */}
      <div className="fixed right-6 top-1/2 -translate-y-1/2 flex flex-col items-center space-y-6 bg-white/10 backdrop-blur-2xl p-4 rounded-[2rem] border border-white/20 shadow-2xl pointer-events-auto transition-all duration-300 hover:bg-white/15">
        ...
        <button
          ...
          title={penEnabled ? 'Disable Drawing' : 'Enable Drawing'}
        >
          {penEnabled ? (
        ...
        <div className="flex flex-col space-y-3">
          {COLORS.map((c) => (
            <button
              key={c}
              onClick={() => setColor(c)}
              className={`w-8 h-8 rounded-full transition-all hover:scale-125 ${
        ...
        <div className="flex flex-col space-y-4">
          {WIDTHS.map((w) => (
            ...
            >
              <div 
                className={`rounded-full transition-all ${width === w ? 'bg-white scale-125' : 'bg-white/30 group-hover:bg-white/50'}`} 
      ...
      {/* User Indicator */}
      <div className="fixed bottom-6 right-6 text-white/20 text-[10px] font-black uppercase tracking-[0.3em]">
